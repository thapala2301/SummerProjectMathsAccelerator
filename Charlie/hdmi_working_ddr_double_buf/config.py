...
DMACR_RUN_PARK = 0x1
DMACR_RESET = 0x4
STATE_NAME = "_camera_ddr_config_state"
...
previous_state = getattr(builtins, STATE_NAME, None)
...
def dump_status():
    ...
    print(f"VDMA PARK_PTR   = 0x{vdma.read(VDMA_PARK_PTR):08x}")
    print(f"VDMA VSIZE      = 0x{vdma.read(MM2S_VSIZE):08x}")
    print(f"VDMA HSIZE      = 0x{vdma.read(MM2S_HSIZE):08x}")
    print(f"VDMA STRIDE     = 0x{vdma.read(MM2S_STRIDE):08x}")
    print(f"VDMA ADDR1      = 0x{vdma.read(MM2S_START_ADDR_1):08x}")
    print(f"VDMA ADDR2      = 0x{vdma.read(MM2S_START_ADDR_2):08x}")
    ...
    print(f"GPIO2           = 0x{gpio.read(GPIO2_DATA):08x}")
    print(f"ACK count       = {globals().get('frame_ack_count', 'n/a')}")
    print(f"ACK active      = {globals().get('frame_ack_active', 'n/a')}")
    print(f"ACK last GPIO   = {globals().get('frame_ack_last_status', 'n/a')}")
...
current_state = {
    "active": True,
    "frame0": renderer_frame0,
    "frame1": renderer_frame1,
}

setattr(builtins, STATE_NAME, current_state)

if previous_state is not None:
    free_buffer(previous_state.get("frame0"))
    free_buffer(previous_state.get("frame1"))
