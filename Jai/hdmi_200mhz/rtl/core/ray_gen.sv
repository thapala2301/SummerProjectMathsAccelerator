...
module ray_gen #(
    ...
    always_ff @(posedge clk) begin
        ...
        s0_id <= pix_id_in;
        s0_lookat <= lookat;
        s0_orig[0] <= (pix_x >= 11'd640) ? cam_origin_right[0] : cam_origin[0];
        s0_orig[1] <= (pix_x >= 11'd640) ? cam_origin_right[1] : cam_origin[1];
        s0_orig[2] <= (pix_x >= 11'd640) ? cam_origin_right[2] : cam_origin[2];
    end
    ...
    logic [10:0] local_px;
    assign local_px = (s0_px >= 11'd640) ? (s0_px - 11'd640) : s0_px;

    always_ff @(posedge clk) begin
        s1_dx <= ({ {20{1'b0}}, local_px, 1'b0 }) - 32'd639;
