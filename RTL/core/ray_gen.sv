...
module ray_gen #(
    ...
    assign s0_right_eye = (pix_x >= (IMG_W/2));
    assign s0_px_eye = s0_right_eye ? (pix_x - (IMG_W/2)) : pix_x;
    ...
    state_pipe_unpacked_array #(.WIDTH(27), .NB_ELEMENTS(9), .DEPTH(DELAY)) pipe_lk (.clk(clk), .in(s0_lookat), .out(lk_dl)); // delay lookat 34 cycles to align with s7 normalised direction
    state_pipe_unpacked_array #(.WIDTH(27), .NB_ELEMENTS(3), .DEPTH(DELAY)) pipe_orig (.clk(clk), .in(s0_orig), .out(orig_dl)); // delay cam_origin 34 cycles to align with matrix multiply input
    ...
    state_pipe_unpacked_array # (.WIDTH(27), .NB_ELEMENTS(3), .DEPTH(12)) u_state_pipe_orig_dl (.clk(clk), .in(orig_dl), .out(orig_final)); // delay cam_origin further 12 cycles to align with matrix multiply output
