...
// march_core.sv: set SCENE_CORE_LAT = 83, instantiate menger_sdf instead of scaffold_sdf
// ctrl.py: scene_cell_sz=2.0, scene_half_cell=1.0, scene_shape_size=1.0 (box half-extent)
//
// All fp_mod replaced with fp_mod2 (mod 2.0 only, 6 cycles vs 52, saves ~9 fp_mod instances)
// All 3 levels computed in PARALLEL from same px/py/pz input.
// fp_min chain split with intermediate register to fix 7.9ns timing violation
// Level 1: fp_mod2(6)+sub(4)+mul(4)+sub(4)+max+reg+min+reg+min+reg+sub(4) = t=25 → state_pipe DEPTH=56
// Level 2: mul(4)+fp_mod2(6)+sub(4)+mul(4)+sub(4)+max+reg+min+reg+min+reg+sub(4)+mul(4) = t=33 → state_pipe DEPTH=48
// Level 3: same as Level 2 → t=33 → state_pipe DEPTH=48
// Box SDF:  abs+sub+max → t=4 → state_pipe DEPTH=77
// All aligned at t=81, combine fp_max×2+reg+fp_max+reg → SCENE_CORE_LAT = 81
...
// Ready at t=4, delayed 76 cycles to align with t=80
// =========================================================
...
wire [26:0] box_d;
state_pipe #(.WIDTH(27), .DEPTH(77)) pipe_box(.clk(clk), .in(box_d_raw), .out(box_d));

// =========================================================
// LEVEL 1 (s=1): fold=mod(p,2)-1, cross, no final fp_mul
// Ready at t=24, delayed 54 cycles to align with t=78
// =========================================================
wire [26:0] lv1_mx, lv1_my, lv1_mz;
// fp_mod inst_lv1_modx(.clk(clk), .a(px), .b(TWO), .rem(lv1_mx)); // old: 52 cycles, ~5800 LUTs each
// fp_mod inst_lv1_mody(.clk(clk), .a(py), .b(TWO), .rem(lv1_my));
// fp_mod inst_lv1_modz(.clk(clk), .a(pz), .b(TWO), .rem(lv1_mz));
fp_mod2 inst_lv1_modx(.clk(clk), .a(px), .rem(lv1_mx)); // t=6: cheap pow2 mod
fp_mod2 inst_lv1_mody(.clk(clk), .a(py), .rem(lv1_my));
fp_mod2 inst_lv1_modz(.clk(clk), .a(pz), .rem(lv1_mz));

wire [26:0] a1x, a1y, a1z;
fp_sub inst_lv1_ax(.clk(clk), .a(lv1_mx), .b(ONE), .out(a1x)); // t=10
...
fp_mul inst_lv1_rmx(.clk(clk), .a(THREE), .b(abs_a1x), .out(r1x_pre)); // t=14
...
fp_sub inst_lv1_rx(.clk(clk), .a(ONE), .b(r1x_pre), .out(r1x)); // t=18
...
fp_abs inst_lv1_arx(.in(r1x), .out(ar1x)); // t=18 combinational
fp_abs inst_lv1_ary(.in(r1y), .out(ar1y));
fp_abs inst_lv1_arz(.in(r1z), .out(ar1z));

wire [26:0] lv1_mxy, lv1_myz, lv1_mzx;
...
reg [26:0] lv1_mxy_r, lv1_myz_r, lv1_mzx_r; // t=19: cut after fp_max
...
wire [26:0] lv1_tmp;
fp_min inst_lv1_mn1(.a(lv1_mxy_r), .b(lv1_myz_r), .out(lv1_tmp));

reg [26:0] lv1_tmp_r; // t=20: cut between fp_min chain
always @(posedge clk) lv1_tmp_r <= lv1_tmp;

wire [26:0] lv1_cross;
fp_min inst_lv1_mn2(.a(lv1_tmp_r), .b(lv1_mzx_r), .out(lv1_cross));

reg [26:0] lv1_cross_r; // t=21: cut after fp_min chain
always @(posedge clk) lv1_cross_r <= lv1_cross;

wire [26:0] c1_raw;
fp_sub inst_lv1_c(.clk(clk), .a(lv1_cross_r), .b(ONE), .out(c1_raw)); // t=25

wire [26:0] c1;
state_pipe #(.WIDTH(27), .DEPTH(56)) pipe_c1(.clk(clk), .in(c1_raw), .out(c1)); // t=81

// =========================================================
// LEVEL 2 (s=3): p*3 -> mod(.,2)-1 -> cross -> /3
// Ready at t=32, delayed 46 cycles to align with t=78
// =========================================================
...
// fp_mod inst_lv2_modx(.clk(clk), .a(px3), .b(TWO), .rem(lv2_mx)); // old: 52 cycles, ~5800 LUTs each
// fp_mod inst_lv2_mody(.clk(clk), .a(py3), .b(TWO), .rem(lv2_my));
// fp_mod inst_lv2_modz(.clk(clk), .a(pz3), .b(TWO), .rem(lv2_mz));
fp_mod2 inst_lv2_modx(.clk(clk), .a(px3), .rem(lv2_mx)); // t=10: cheap pow2 mod
fp_mod2 inst_lv2_mody(.clk(clk), .a(py3), .rem(lv2_my));
fp_mod2 inst_lv2_modz(.clk(clk), .a(pz3), .rem(lv2_mz));

wire [26:0] a2x, a2y, a2z;
fp_sub inst_lv2_ax(.clk(clk), .a(lv2_mx), .b(ONE), .out(a2x)); // t=14
...
fp_mul inst_lv2_rmx(.clk(clk), .a(THREE), .b(abs_a2x), .out(r2x_pre)); // t=18
...
fp_sub inst_lv2_rx(.clk(clk), .a(ONE), .b(r2x_pre), .out(r2x)); // t=22
...
wire [26:0] lv2_mxy, lv2_myz, lv2_mzx;
...
reg [26:0] lv2_mxy_r, lv2_myz_r, lv2_mzx_r; // t=23: cut after fp_max
always @(posedge clk) begin
    lv2_mxy_r <= lv2_mxy;
    lv2_myz_r <= lv2_myz;
    lv2_mzx_r <= lv2_mzx;
...
wire [26:0] lv2_tmp;
fp_min inst_lv2_mn1(.a(lv2_mxy_r), .b(lv2_myz_r), .out(lv2_tmp));

reg [26:0] lv2_tmp_r; // t=24: cut between fp_min chain
always @(posedge clk) lv2_tmp_r <= lv2_tmp;

wire [26:0] lv2_cross;
fp_min inst_lv2_mn2(.a(lv2_tmp_r), .b(lv2_mzx_r), .out(lv2_cross));

reg [26:0] lv2_cross_r; // t=25: cut after fp_min chain
always @(posedge clk) lv2_cross_r <= lv2_cross;

wire [26:0] c2_sub;
fp_sub inst_lv2_c(.clk(clk), .a(lv2_cross_r), .b(ONE), .out(c2_sub)); // t=29

wire [26:0] c2_raw;
fp_mul inst_lv2_inv(.clk(clk), .a(c2_sub), .b(INV3), .out(c2_raw)); // t=33

wire [26:0] c2;
state_pipe #(.WIDTH(27), .DEPTH(48)) pipe_c2(.clk(clk), .in(c2_raw), .out(c2)); // t=81
...
// fp_mod inst_lv3_modx(.clk(clk), .a(px9), .b(TWO), .rem(lv3_mx)); // old: 52 cycles, ~5800 LUTs each
// fp_mod inst_lv3_mody(.clk(clk), .a(py9), .b(TWO), .rem(lv3_my));
// fp_mod inst_lv3_modz(.clk(clk), .a(pz9), .b(TWO), .rem(lv3_mz));
fp_mod2 inst_lv3_modx(.clk(clk), .a(px9), .rem(lv3_mx)); // t=10: cheap pow2 mod
fp_mod2 inst_lv3_mody(.clk(clk), .a(py9), .rem(lv3_my));
fp_mod2 inst_lv3_modz(.clk(clk), .a(pz9), .rem(lv3_mz));
...
wire [26:0] lv3_mxy, lv3_myz, lv3_mzx;
...
reg [26:0] lv3_mxy_r, lv3_myz_r, lv3_mzx_r; // t=23: cut after fp_max
always @(posedge clk) begin
    lv3_mxy_r <= lv3_mxy;
    lv3_myz_r <= lv3_myz;
    lv3_mzx_r <= lv3_mzx;
...
wire [26:0] lv3_tmp;
fp_min inst_lv3_mn1(.a(lv3_mxy_r), .b(lv3_myz_r), .out(lv3_tmp));

reg [26:0] lv3_tmp_r; // t=24: cut between fp_min chain (~7.9ns path)
always @(posedge clk) lv3_tmp_r <= lv3_tmp;

wire [26:0] lv3_cross;
fp_min inst_lv3_mn2(.a(lv3_tmp_r), .b(lv3_mzx_r), .out(lv3_cross));

reg [26:0] lv3_cross_r; // t=25: cut after fp_min chain
always @(posedge clk) lv3_cross_r <= lv3_cross;

wire [26:0] c3_sub;
fp_sub inst_lv3_c(.clk(clk), .a(lv3_cross_r), .b(ONE), .out(c3_sub)); // t=29

wire [26:0] c3_raw;
fp_mul inst_lv3_inv(.clk(clk), .a(c3_sub), .b(INV9), .out(c3_raw)); // t=33

wire [26:0] c3;
state_pipe #(.WIDTH(27), .DEPTH(48)) pipe_c3(.clk(clk), .in(c3_raw), .out(c3)); // t=81

// =========================================================
// COMBINE: d = max(box, max(-c1, max(-c2, -c3)))
// All signals aligned at t=78. Negate = flip sign bit in FP27.
// CSG subtraction: max(box, -cross) = box minus fractal holes
// 3 chained fp_max is ~4.5ns combinational — split into 2 reg stages
// t=81: fp_max x2 -> reg at t=82, fp_max x1 -> reg at t=83 = SCENE_CORE_LAT
// =========================================================
wire [26:0] neg_c1 = {~c1[26], c1[25:0]};
wire [26:0] neg_c2 = {~c2[26], c2[25:0]};
wire [26:0] neg_c3 = {~c3[26], c3[25:0]};

wire [26:0] stage1, stage2;
fp_max inst_fin1(.a(box_d), .b(neg_c1), .out(stage1));
fp_max inst_fin2(.a(stage1),.b(neg_c2), .out(stage2));

reg [26:0] stage2_r; // t=79: cut after first 2 fp_max
always @(posedge clk) stage2_r <= stage2;

wire [26:0] sdf_comb;
fp_max inst_fin3(.a(stage2_r),.b(neg_c3), .out(sdf_comb));

always @(posedge clk) sdf_out <= sdf_comb; // t=83 = SCENE_CORE_LAT
