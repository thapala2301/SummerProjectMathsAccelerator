...
# Buttons via AXI GPIO at 0x41210000 — initialized after Overlay() in run_selector()
...
OPTIONS = [
    ...
    ("Twisted Torus", "Bitstream_Handoff/torus.bit", True),
    ...
    ("Menger Sponge",         "Bitstream_Handoff/MengerSponge.bit",         True),
    ("Mandelbox",     "Bitstream_Handoff/Mandelbox.bit",     False),
    ("Sierpinski Tetrahedron",        "Bitstream_Handoff/SierpinskiTetrahedron.bit",        False)
]
...
def run_selector(framebuf):
    global _btn_mmio
    _btn_mmio = MMIO(0x41210000, 0x1000)  # UI bitstream must be loaded before this
...
def start_ui_vdma(ol=None, prev_frames=None):
    if prev_frames is not None:
        prev_frames[0].freebuffer()
        prev_frames[1].freebuffer()
    if ol is None:
        ol = Overlay("ui.bit")
...
if __name__ == "__main__":
    ...
    ol, framebuf, framebuf1 = start_ui_vdma()   # first boot — monitor already cold, no kill needed
    ...
    while True:
        ...
        _in_raymarcher = True
        try:
            # reuse UI frame buffers — allocate() fails after SDF overlay loads
            ctrl.run(stop_event=stop, music_on=music_on, frame0=framebuf, frame1=framebuf1)
        ...
        ol, framebuf, framebuf1 = start_ui_vdma(ol)
