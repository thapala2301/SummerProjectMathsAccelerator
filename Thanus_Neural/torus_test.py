# This one is for a torus - 'Level 2' in difficulty or sumn
...
def torus_sdf(points, R=1.0, r=0.3):
    q = np.sqrt(points[:,0]**2 + points[:,1]**2) - R
    return np.sqrt(q**2 + points[:,2]**2) - r

distances = torus_sdf(points)
...
torch.save(model.state_dict(), 'torus_sdf.pth')
