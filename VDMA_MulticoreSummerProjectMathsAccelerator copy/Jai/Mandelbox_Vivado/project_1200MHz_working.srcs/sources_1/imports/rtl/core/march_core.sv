module march_core(
    ...
    input  logic [26:0] cam_origin_right [0:2],
...
localparam int RG_LAT = 60;
localparam int SDF_LAT = 291; // corrected for actual latencies: fp_mul=5, fp_add=5, fp_isqrt=20, fp_length=40
localparam int STEP_LAT = 10;
...
wire        s1_is_new_ray_comb = (d1_iter == 8'd0);
...
logic [26:0] new_pos_x;
logic [26:0] new_pos_y;
logic [26:0] new_pos_z;
logic [26:0] d2_pos_x_d;
logic [26:0] d2_pos_y_d;
logic [26:0] d2_pos_z_d;

state_pipe #(.WIDTH(27), .DEPTH(5)) inst1_d2_pos_x_d(.clk(clk), .in(d2_pos_x), .out(d2_pos_x_d));
state_pipe #(.WIDTH(27), .DEPTH(5)) inst1_d2_pos_y_d(.clk(clk), .in(d2_pos_y), .out(d2_pos_y_d));
state_pipe #(.WIDTH(27), .DEPTH(5)) inst1_d2_pos_z_d(.clk(clk), .in(d2_pos_z), .out(d2_pos_z_d));
