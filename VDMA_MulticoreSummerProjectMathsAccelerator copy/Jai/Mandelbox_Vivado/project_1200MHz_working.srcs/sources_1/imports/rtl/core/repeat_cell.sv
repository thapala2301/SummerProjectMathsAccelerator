...
state_pipe #(.WIDTH(27), .DEPTH(6)) pipe_p_d4    (.clk(clk), .in(p),               .out(p_d4));
state_pipe #(.WIDTH(1),  .DEPTH(6)) pipe_sign_d4 (.clk(clk), .in(p[26]),           .out(p_neg_d4));
state_pipe #(.WIDTH(1),  .DEPTH(6)) pipe_wrap_d4 (.clk(clk), .in(|wrap_off_comb),  .out(p_wrap_d4));
