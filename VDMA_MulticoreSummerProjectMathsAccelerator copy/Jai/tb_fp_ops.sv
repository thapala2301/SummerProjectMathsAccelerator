...
//FP27 constants (sign | exponent | fraction)
...
localparam logic [26:0] FP_THREE   = 27'h2020000; // 3.0 = 1.5 * 2^1
localparam logic [26:0] FP_HALF    = 27'h1F80000; // 0.5
localparam logic [26:0] FP_NEG_HALF= 27'h5F80000; // -0.5
localparam logic [26:0] FP_NEG_ONE = 27'h5FC0000; // -1.0
localparam logic [26:0] FP_1P5     = 27'h1FE0000; // 1.5
localparam logic [26:0] FP_3P5     = 27'h2030000; // 3.5 = 1.75 * 2^1
localparam logic [26:0] FP_4P5     = 27'h2048000; // 4.5
localparam logic [26:0] FP_FOUR    = 27'h2040000; // 4.0

task check_fp(input string label, input logic [26:0] got, input real expected, input real tol);
    real g, err;
    g = fp27(got);
    err = g - expected; if (err < 0) err = -err;
    if (err <= tol)
...
initial begin
    ...
    // fp_mul tests
    $display("\nTEST fp_mul (2 cycle latency)");

    // 2.0 * 3.0 = 6.0
    mul_a <= FP_TWO; mul_b <= FP_THREE;
    @(posedge clk); @(posedge clk); @(posedge clk); // wait 2 cycles + 1 to sample
    ...
    // fp_isqrt tests
    $display("\nTEST fp_isqrt (10 cycle latency)");

    // 1/sqrt(1.0) = 1.0
    // present input for 1 cycle, then hold zero so pipeline sees only one valid input
    ...
    // int2fp tests (combinatorial)
    $display("\nTEST int2fp (combinatorial)");

    int_in = 32'sd536870912;  #1; // 1.0 * 2^29
    check_fp("int2fp(1.0)",  int_out, 1.0, 0.01);

    int_in = 32'sd268435456;  #1; // 0.5 * 2^29
