...
logic sys_clk;
logic clk_pixel;
logic clk_tmds_5x; //pixel clock x 5 in speed: HDMI encodes each color channel as TMDS serial data
//that's 10 bits per px period serialized over wire. Running at 5x allowed 2 bits per clk_tmds_5x cycle using DDR, getting 10 bit serialisation
logic locked;
logic btn_reset;
logic rst;
...
logic [26:0] lookat_frame[0:8]; //snapshot copies of cam_origin and lookat taken at start of each frame.
//python updates cam pos continuosly, need not to change mid render, so moemnt dispatch starts for new frame, latch cam origin 
//won't affect curr frame till next boundary
