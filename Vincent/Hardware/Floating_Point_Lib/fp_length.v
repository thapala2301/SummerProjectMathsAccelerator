...
//fp_mul=4, fp_add=4, fp_isqrt=16
//cycle 0:  x,y,z in
//cycle 4:  x2,y2,z2 ready
//cycle 8:  x2+y2 ready; z2_delayed arrives (DEPTH=4)
//cycle 12: sum = x2+y2+z2 ready
//cycle 28: isqrt(sum) ready (fp_isqrt=16)
//sum_delayed DEPTH=16 so it arrives at cycle 28 alongside isqrt
//Cycle 32: sqrt_sum = isqrt * sum ready (fp_mul=4)
...
state_pipe #(.WIDTH(27), .DEPTH(16)) pipe_sum(.clk(clk), .in(sum_x2_y2_z2), .out(sum_delayed));
