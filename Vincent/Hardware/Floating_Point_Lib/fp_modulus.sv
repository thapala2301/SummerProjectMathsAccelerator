...
logic [26:0] b_d, a_d, floor_a_b, b_floor, ad_d, a_b, b_floor_div;
fp_division u_dividea_b(.clk(clk), .a(a), .b(b), .a_over_b(a_b)); //uses a and b, obtain a/b - takes 36 clk
fp_floor u_floora_b (.clk(clk), .a(a_b), .floor_a(floor_a_b)); //takes 0 clk, comb, gets floor(a/b)

state_pipe #(.WIDTH(27), .DEPTH(40)) u_state_pipe_a (.clk(clk), .in(b), .out(b_d));

fp_mul u_mula_b(.clk(clk), .a(b_d), .b(floor_a_b), .out(b_floor_div)); //gets b* floor(a/b)

state_pipe #(.WIDTH(27), .DEPTH(44)) u_state_pipe_a2 (.clk(clk), .in(a), .out(a_d));
