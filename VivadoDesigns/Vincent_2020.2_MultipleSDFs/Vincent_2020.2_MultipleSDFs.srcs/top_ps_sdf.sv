...
logic pl_axi_aclk, pl_axi_aresetn;
logic pixel_clk_out, pixel_aresetn_out;
logic [242:0] lookat_flat;
logic [80:0]  cam_origin_flat;
logic [31:0]  frame_base_0, frame_base_1;
logic [23:0]  bd_vid_data;
logic [31:0]  vid_data;
logic         vid_hsync, vid_vsync, vid_active;
...
logic         m_axi_fb_awvalid, m_axi_fb_awready;
logic [63:0]  m_axi_fb_wdata;
logic [7:0]   m_axi_fb_wstrb;
logic         m_axi_fb_wlast, m_axi_fb_wvalid, m_axi_fb_wready;
logic [1:0]   m_axi_fb_bresp;
logic         m_axi_fb_bvalid, m_axi_fb_bready;
logic [0:0]   bd_frame_ack;
logic [31:0]  debug_status;

assign vid_data  = {8'd0, bd_vid_data};

control_bd_wrapper inst_bd (
    ...
    .S_AXI_FB_WR_araddr(32'd0), .S_AXI_FB_WR_arburst(2'b01),
    .S_AXI_FB_WR_arcache(4'd0), .S_AXI_FB_WR_arlen(8'd0),
    .S_AXI_FB_WR_arlock(1'b0), .S_AXI_FB_WR_arprot(3'd0),
    .S_AXI_FB_WR_arqos(4'd0), .S_AXI_FB_WR_arready(),
    .S_AXI_FB_WR_arsize(3'd3), .S_AXI_FB_WR_arvalid(1'b0),
    ...
    .S_AXI_FB_WR_rdata(), .S_AXI_FB_WR_rlast(),
    .S_AXI_FB_WR_rready(1'b1), .S_AXI_FB_WR_rresp(), .S_AXI_FB_WR_rvalid(),
    ...
    .pl_axi_aclk(pl_axi_aclk), .pl_axi_aresetn(pl_axi_aresetn),
    .pixel_clk_in(pixel_clk_out), .pixel_aresetn_in(pixel_aresetn_out),
    .lookat_flat(lookat_flat), .cam_origin_flat(cam_origin_flat),
    .frame_base_0(frame_base_0), .frame_base_1(frame_base_1),
    .vid_data(bd_vid_data), .vid_hsync(vid_hsync),
    .vid_vsync(vid_vsync), .vid_active(vid_active),
    .frame_ready_valid(debug_status[1]),
    .frame_ready_bank(debug_status[0]),
...
top inst_top (
    ...
    .lookat_flat(lookat_flat), .cam_origin_flat(cam_origin_flat),
    .frame_base_0(frame_base_0), .frame_base_1(frame_base_1),
    .ctrl_clk_out(pl_axi_aclk), .ctrl_aresetn_out(pl_axi_aresetn),
    .pixel_clk_out(pixel_clk_out), .pixel_aresetn_out(pixel_aresetn_out),
    .vid_data(vid_data), .vid_hsync(vid_hsync),
    .vid_vsync(vid_vsync), .vid_active(vid_active),
    .m_axi_fb_awaddr(m_axi_fb_awaddr), .m_axi_fb_awlen(m_axi_fb_awlen),
    .m_axi_fb_awsize(m_axi_fb_awsize), .m_axi_fb_awburst(m_axi_fb_awburst),
    .m_axi_fb_awcache(m_axi_fb_awcache), .m_axi_fb_awprot(m_axi_fb_awprot),
    .m_axi_fb_awvalid(m_axi_fb_awvalid), .m_axi_fb_awready(m_axi_fb_awready),
    .m_axi_fb_wdata(m_axi_fb_wdata), .m_axi_fb_wstrb(m_axi_fb_wstrb),
    .m_axi_fb_wlast(m_axi_fb_wlast), .m_axi_fb_wvalid(m_axi_fb_wvalid),
    .m_axi_fb_wready(m_axi_fb_wready),
    .m_axi_fb_bresp(m_axi_fb_bresp), .m_axi_fb_bvalid(m_axi_fb_bvalid),
    .m_axi_fb_bready(m_axi_fb_bready),
    .frame_ack(bd_frame_ack[0]), .debug_status(debug_status),
