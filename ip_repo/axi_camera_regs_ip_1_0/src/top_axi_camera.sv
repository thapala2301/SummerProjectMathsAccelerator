...
genvar gi;
generate
    for (gi = 0; gi < 9; gi++) begin : gen_lookat_unpack
        assign lookat[gi] = lookat_flat[gi*FP_W +: FP_W];
    end
    for (gi = 0; gi < 3; gi++) begin : gen_origin_unpack
        assign cam_origin[gi] = cam_origin_flat[gi*FP_W +: FP_W];
    end
endgenerate
